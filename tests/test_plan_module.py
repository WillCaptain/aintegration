...
async def test_task_status_transitions(plan_module):
    ...
    assert not task.can_transition_to(TaskStatus.DONE.value)  # 从NotStarted不能直接到Done
    ...
    # 测试完成状态不能转换
    assert not task.can_transition_to(TaskStatus.RUNNING.value)
...
async def test_listener_trigger_conditions(plan_module):
    ...
    assert listener.evaluate_action_condition({})
